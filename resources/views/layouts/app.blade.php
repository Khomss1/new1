...
<head>
    ...
    <title>@yield('title', 'e-Monev 2026')</title>
    ...
    <div class="flex flex-col md:flex-row">
        ...
        <aside class="w-full md:w-64 bg-slate-900 text-white flex-shrink-0 hidden md:flex flex-col h-screen sticky top-0">
            ...
            <nav class="flex-1 overflow-y-auto py-4">
                ...
                <!-- Dashboard -->
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-6 py-3 text-sm font-medium transition-colors hover:bg-slate-800 {{ request()->is('admin/dashboard') ? 'bg-blue-900 border-r-4 border-yellow-500' : 'text-gray-300' }}">
                    <i class="ph ph-squares-four text-xl"></i> Dashboard
                </a>

                <!-- Data BP -->
                <a href="{{ route('admin.bp.index') }}" class="flex items-center gap-3 px-6 py-3 text-sm font-medium transition-colors hover:bg-slate-800 {{ request()->is('admin/bp*') ? 'bg-blue-900 border-r-4 border-yellow-500' : 'text-gray-300' }}">
                    <i class="ph ph-users-three text-xl"></i> Data Badan Publik
                </a>

                <!-- Hasil Kuesioner -->
                <a href="{{ route('admin.evaluasi.index') }}" class="flex items-center gap-3 px-6 py-3 text-sm font-medium transition-colors hover:bg-slate-800 {{ request()->is('admin/evaluasi*') ? 'bg-blue-900 border-r-4 border-yellow-500' : 'text-gray-300' }}">
